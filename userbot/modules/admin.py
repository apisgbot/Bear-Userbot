...
NO_ADMIN = "`Baginda Bukan Admin 🗿`"
NO_PERM = "`Baginda Tidak Mempunyai Izin!`"
NO_SQL = "`Berjalan Pada Mode Non-SQL`"

CHAT_PP_CHANGED = "`Berhasil Mengubah Profil Grup`"
CHAT_PP_ERROR = (
    "`Ada Masalah Dengan Memperbarui Foto,`"
    "`Mungkin Karna Baginda Bukan Admin,`"
...
async def rm_deletedacc(show):
    ...
    # Well
    if not admin and not creator:
        return await show.edit("`Baginda Bukan Admin Disini! 🗿`")

    await show.edit("`Menghapus Akun Terhapus...\nMohon Menunggu Baginda Sedang Dalam Proses`")
    del_u = 0
    del_a = 0
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                return await show.edit("`Baginda Tidak Memiliki Izin Banned Dalam Grup Ini`")
